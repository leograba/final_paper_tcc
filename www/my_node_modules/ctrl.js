...
function spargingControl(recipeContents){
	...
	var spargeTimer = setInterval(function(){//handles the sparging process
		...
		if(!mashTunOverflow && (global.environmentVariables.timestamps.curr - global.environmentVariables.timestamps.startDrain) > drainTimeSetp){
			//if it is time to start spraying the sparge water, and there is no overflow
			gpioCfg.changeStatusIO("boil_valve", "true");//open the drain
		...
		else if(!mashTunOverflow){//if it is running nicely for sure
			...
			if(realSpargeTime > spargeTimeSetp){//if the running time is enough
				...
				gpioCfg.changeStatusIO("boil_valve", "false");//close the drain
		...
		if(!mashTunOverflow && global.environmentVariables.ioStatus["led"].val == b.HIGH){//if MT is about to overflow
