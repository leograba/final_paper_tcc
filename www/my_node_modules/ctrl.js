	...
	if(global.environmentVariables.okToStart){//first check is if the recipe is ok to start
		if(recipe == global.environmentVariables.recipe){//recipe name should match also
			fs.writeFile(lockFile, 1, function(err){//write to lockfile telling there is a recipe in progress
				...
				global.environmentVariables.auto = true;//user unable to control the process
...
function heatMashWater(recipeContents){
	...
	var readTmpTimer = setInterval(function(){//read temperature every second
		...
		log.getTemperatureReading(instantPath, logReadHandler, function(err, temperature){
			...
			updateHeatingConditions("MT", mashSetpoint, temperature, function(err, heatingStatus){
				if(err) throw err;//developer mustn't set wrong vessel
				if(!reachedSetpoint && !heatingStatus){//if it is the first time the setpoint is reached
	...
	var readTmpTimer = setInterval(function (){//read temperature every second
		...
		if(global.environmentVariables.readyForNextStep && currentSetpointPointer < 8){//if there are still mashing steps
			...
			log.getTemperatureReading(instantPath, logReadHandler, function(err, temperature){//read current temperature
				...
				updateHeatingConditions("MT", mashSetpoint, temperature, function(err, heatingStatus){//control temperature for current setpoint
					if(err) throw err;//developer mustn't set wrong vessel
					if(!finishedRamp && !heatingStatus){//if the ramp just reached its setpoint (first time heatingStatus return false)
					...
					//do it if heating or not, doesn't matter
					else if(finishedRamp && (global.environmentVariables.timestamps.curr - global.environmentVariables.timestamps["fRamp" + currentSetpointPointer]) > stepTime){
						//as security should check if the fRamp timestamp is already defined
						//if it isn't a ramp and the step rest is finished
						finishedRamp = false;
					...
					else if(finishedRamp){//if it is in the middle of a step, heat the sparging water and tell the user the step time left
						...
						debug("Step time left: " + global.environmentVariables.timeLeft);
						if(spargeSet){//only heat sparging water if there is sparging water to heat
							log.getTemperatureReading(instantBK, logReadHandlerBK, function(err, temperature){//read current temperature
								...
								updateHeatingConditions("BK", spargeSetpoint, temperature, function(err, heatingStatus){//heat the sparging water
									if(err) throw err;//developer mustn't set wrong vessel
									//maybe should sinalize somehow when the sparge water is heated
								});
							});
						}
					}
	...
	var spargeTimer = setTimeout(function(){//wait until the first mash drain is finished before spraying sparge water
		...
		spargeTimer = setInterval(function(){//must wait for the sparge time in a loop because of the overflow check
			if(!mashTunOverflow){//if there is no overflow
				...
				debug("Tempo de sparging: " + realSpargeTime);
...
function theBoil(recipeContents){
	...
	var hopPointer = 0;//used in for loop to be passed to setTimeout
	var closeHopboxTime = 5000; //wait 2.5 seconds in every hop addition before closing the hop box
	...
	var readTmpTimer = setInterval(function(){//read temperature every second
		...
		log.getTemperatureReading(instantBK, logReadHandlerBK, function(err, temperature){
			...
			if(temperature > 96 && !boilFlag){//if it is near enough boiling
				...
				clearInterval(logTimer);//stop logging code 8
				...
				endBoilTime = 60*1000*recipeContents[6] + global.environmentVariables.timestamps.curr;//get the end of boiling time
				global.environmentVariables.timestamps.boilFinishScheduled = endBoilTime;
				for(hopPointer = 0; hopPointer < 8; hopPointer++){//check all possible hop addition times
					...
					//this way, if the hops are out of order of addition, it doesn't matter
						...
						(function (hPointer){//hPointer is the copy of hopPointer when function is invoked, different name just to let it explicit
							//recipeContents doesn change, so no need to copy
							setTimeout(function(){//then add it when it is time
								debug("Hop pointer closure: " + hPointer);
								debug("Adding hop " + recipeContents[40 + 3*hPointer]);
								controlHopCompartment(hPointer + 1);
								log.logToFile("added hop [" + hPointer + "]", 10, "hAdd" + hPointer);//log the addition as a notable timestamp
								setTimeout(function(){//close the hops compartment every addition
									debug("Hop " + recipeContents[40 + 3*hPointer] + " added");
									controlHopCompartment(0);
								},closeHopboxTime);
							},hopAddSetp);
						})(hopPointer);
						/*
						setTimeout(function(){//then add it when it is time
							debug("Hop pointer: " + hopPointer);
							debug(recipeContents);
							debug("Adding hop " + recipeContents[40 + 3*hopPointer]);
							controlHopCompartment(hopPointer + 1);
							log.logToFile("added hop [" + hopPointer + "]", 10, "hAdd" + hopPointer);//log the addition as a notable timestamp
							setTimeout(function(){//close the hops compartment every addition
								debug("Hop " + recipeContents[40 + 3*hopPointer] + " added");
								controlHopCompartment(0);
							},closeHopboxTime);
						},hopAddSetp);*/
					}
				}
				boilFlag = true;//prevent from doing it all again
			}
		});
		if(global.environmentVariables.timestamps.curr > endBoilTime){//end of the boil
			debug("end of the boil");
			clearInterval(readTmpTimer);//stop the boiling
			clearInterval(logTimer);//stop logging boiling process
			...
			chillRecirculation(recipeContents);//recirculate the wort through the chiller before adding the yeast
		}
		else{//just for information sake
			debug("Boil time left: " + (endBoilTime - global.environmentVariables.timestamps.curr));
		}
	},1000);
}

function chillRecirculation(recipeContents){
	//just pass the wort through the chiller
	//in a perfect world, would regulate the flow so that the wort is being chilled to a certain temperature
	//Must prompt the user if the cleaning process must be started
	
	//var drainageTimeSetp = 500*recipeContents[3];//time that this process should go on
	var drainageTimeSetp = 1000*60*2;//two seconds for debbuging purposes
	var lockFile = "./datalog/lockfile";//file that tells if recipe is running
	
	log.logToFile("chilling the wort", 11, "chillStart");//log the process start
	var logTimer = setInterval(function(){//log while chilling
		log.logToFile("chilling the wort", 11);//log periodically
	},5000);
	
	gpioCfg.changeStatusIO("boil_valve", "true");//open all valves leading to the chiller
	gpioCfg.changeStatusIO("chill_valve", "true");
	gpioCfg.changeStatusIO("water_valve", "true");
	debug("Starting to chill the wort. Estimated time: " + drainageTimeSetp);
	setTimeout(function(){//shutdown everything after chilling the wort
		debug("Finished chilling the wort.");
		gpioCfg.changeStatusIO("boil_valve", "false");//close all valves leading to the chiller
		gpioCfg.changeStatusIO("chill_valve", "false");
		gpioCfg.changeStatusIO("water_valve", "false");
		clearInterval(logTimer);//stop logging the chilling process
		log.logToFile("chill finished - brewing finished", 12, "end");//log the process end
		logTimer = setInterval(function(){//log until user enable to continue
			log.logToFile("waiting for the user ok before the cleaning", 12);//log periodically
		},5000);
		var waitTimer = setInterval(function(){
			if(global.environmentVariables.readyForNextStep){//when user send OK
				global.environmentVariables.readyForNextStep = false;//ok I know it, now prepare for next step
				clearInterval(logTimer);//stop logging
				clearInterval(waitTimer);//stop query for the user input
				fs.writeFile(lockFile, 0, function(err){//release system - now it is up to the user not to start new recipe
					if(err) return;
				});
				doClean();//start cleaning process
			}
		},500);
	},drainageTimeSetp);
}
...
function doClean(){
	//Start the cleaning process, but some part is up to the user
	//this function is just turning things on and off, and timing
	var timeMT = 1000*60;//*5;//5 minute
	var timeBK = 1000*60;//*5;//5 minute
	var timeChiller = 1000*60;//*5;//5 minute
	var timeDrain = 1000*60;//*30;//30 minute
	
	debug("Start recirculation of water to clean the equipment");
	log.logToFile("cleaning recirculation", 13, "cleaningStart");//log the process end
	var logTimer = setInterval(function(){//log until user enable to continue
		log.logToFile("cleaning recirculation", 13);//log periodically
	},5000);
	//for the MT loop:
	debug("Mash tun loop");
	gpioCfg.changeStatusIO("mash_pump", "true");
	setTimeout(function(){//after the MT loop
		debug("Brewing kettle loop");
		gpioCfg.changeStatusIO("mash_pump", "false");
		gpioCfg.changeStatusIO("mash_valve", "true");
		gpioCfg.changeStatusIO("boil_pump", "true");
		gpioCfg.changeStatusIO("boil_valve", "true");
		setTimeout(function(){//after the BK loop
			//leave mash_valve open so it can completely drain
			debug("Chiller loop");
			gpioCfg.changeStatusIO("chill_valve", "true");
			setTimeout(function(){//shutdown the pump or it will work without liquid
				debug("Wait for everything to drain");
				gpioCfg.changeStatusIO("boil_pump", "false");
				setTimeout(function(){//wait a lot so everything can drain
					gpioCfg.changeStatusIO("mash_valve", "false");//then shutdown
					gpioCfg.changeStatusIO("boil_valve", "false");
					gpioCfg.changeStatusIO("chill_valve", "false");
					clearInterval(logTimer);
					global.environmentVariables.auto = false;//let the user take over
					global.environmentVariables.code = "";//tells the GUI that the system is idle
					debug("Finally nothing else to do! Start again if you wish!");
					//must save the log file for statistics etc
					//must change the log number so the web page presents the correct message
					//think: what else needs to be done aside from closing all valves etc?
				},timeDrain);
			},timeChiller);
		},timeBK);
	},timeMT);
...
function updateHeatingConditions(vessel, setp, temperature, callbackUpdateHeating){
	//This function must be called in a loop, every loop interation
	//callback(err, value) returns true if heating, false if setpoint reached
	if(vessel == "mash_heat" || vessel == "MT" || vessel == "mash tun"){
		...
		callbackUpdateHeating("unknown vessel", null);
	...
	if(temperature < 0.7*setp){//if temperature is less then 0.7 of the setpoint
		...
		callbackUpdateHeating(null, true);//pass true if heating
	}
		...
		callbackUpdateHeating(null, false);//pass false if not heating
	}
}