				...
				if(lastReadingsTimestamp[0] >= lastReadingsTimestamp[4]){//if the reading is the same within 4s
					...
					if(parsedData[0] < 0.7*mashSetpoint){//if temperature is less then 0.7 of the setpoint
						...
						if(!reachedSetpoint){//if it is the first time the setpoint is reached
							...
							debug(	"            Current emperature = " + parsedData[0] + "\n"
						...
						if(global.environmentVariables.readyForNextStep){
...
function rampControl(recipeContents){
	...
	for(var i = 0; i < 8; i++){//there may be 8 steps or less
		
	}
}

function getTemperatureReading(logFilePath, callback){
	fs.readFile(logFilePath, "utf-8" ,function(err, data){//gets the most recent temperature reading
		...
		if(err){//could not read temperature
			...
			callback(null, parsedData[0]);//finished successfully to get the temperature reading
			}
		}
	});
}