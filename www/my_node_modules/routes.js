...
module.exports.configRoute = function (req, res) {
	...
	if(request == "datetime"){//send the system time/date to the client
		...
		//only let user do it if ntp is down
		exec('ntpq -p | tail -n +3 | cut -c 50-53 | grep -v 0',
		function(error, stdout, stderr){
			//if command cannot be executed or servers are all down, user can change date/time
			if(stdout && !error){
				debug('ntp is running, client is not allowed to change date/time');//print error
				serverResponse.datetime = "denied";//client denied to take this action
				res.send(serverResponse);
			}
			else{
				exec('sudo date -s ' + nd + ' | hwclock -w',//set date
				function(error, stdout, stderr) {
...
module.exports.clientrequestRoute = function(req, res){
	var command = req.body.command;
	var serverResponse = {resp:"success - going to next step"};
	debug("Client request: " + command);
	if(command == "ready"){//set flag to tell ctrl.startMashingProcess() to go to the next step
