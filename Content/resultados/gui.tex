...
Observações do painel de controle decorrentes da simulação em bancada documentada na seção \ref{protosim_sec}, mostraram que por vezes o estado do sistema na GUI fica defasado com relação ao estado do sistema na BBB e no hardware de interface. No modo manual, \textit{glitches} na GUI foram observados com frequência ao mudar o estado dos pinos de GPIO ou do PWM. Ambas as situações se devem ao fato de que a taxa de atualização do painel de controle é de 0,5 segundos --- comprovada quando a taxa de atualização foi modificada temporariamente para 0,1 segundos e os atrasos e \textit{glitches} cessaram. Outro ponto notável do painel de controle do sistema é que não foi implementado nenhum tipo de restrição com relação a quem pode acessar este recurso --- com exceção do fato de que, se o serviço de \textit{no-ip} fosse desativado, somente usuários conectados na rede LAN poderiam acessar a GUI.
...
Para testar a criação de uma nova receita, foi escolhido o nome "\textit{Nov@? Re\$\$eita\#}", que é um nome inválido dado o uso de caracteres especiais. O resultado é apresentado na figura \ref{nome_invalido}. A partir destes resultados, percebe-se que o esforço de proteção aplicado ao servidor e descrito na seção \ref{recipe_editor_howto}, que impediu esta receita de ser salva, não foi correspondido do lado do cliente, permitindo a este realizar uma operação ilegal. Nota-se também na figura \ref{nome_invalido} (d) que caracteres especiais foram passados para a URL do editor de receitas.
...
Após a verificação de que foi possível a edição de uma receita com nome inválido, mas sem a possibilidade de salvá-la, foram executados testes para uma receita cujo nome escolhido foi "\textit{Exemplo American Stout}" e cujo conteúdo inserido apresentou parâmetros compatíveis com uma receita real. Do ponto de vista do cliente, foi comprovado o funcionamento da função recursiva que adiciona campos novos à medida que a receita é preenchida, conforme explicado na seção \ref{recipe_editor_howto} e ilustrado na figura \ref{new_fields}, que compara uma receita vazia à "\textit{Exemplo American Stout}".

\begin{figure}[H]
	\centering
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=4.5cm]{./Resources/uips/empty_recipe.png}
		\caption{Todos os campos em branco}
		\label{new_fields:1}
	\end{subfigure}
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=4.5cm]{./Resources/uips/full_recipe.png}
		\caption{Após preenchimento de uma receita real}
		\label{new_fields:2}
	\end{subfigure}
	\captionsetup{justification=centering}
	\caption[Editor de receitas]{Editor de receitas}
	\label{new_fields}
\end{figure}  

Uma vez que a função recursiva é executada na máquina do cliente, a partir das ferramentas de desenvolvedor do navegador Google Chrome foi observado o comportamento da execução do código javascript durante o evento de perda de foco de um campo da seção maltes do formulário. Esta seção foi escolhida devido ao fato de ela ter 7 linhas de ingredientes preenchidas no momento do teste e, portanto, apresentar a condição que demanda o maior número de chamadas possível da função recursiva \textit{rearrange}. O resultado é apresentado na figura \ref{client_rearrange}. A máquina que foi utilizada para este teste possui 4 núcleos Intel Core i5 M 580 @ 2,67GHz e dois pentes de memória RAM SODIMM DDR3 4GB @ 1066MHz.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.40]{./Resources/uips/rearrange_client.png}
	\captionsetup{justification=centering}
	\caption[Análise de execução do código Javascript da interface de edição de receitas]{Análise de execução do código Javascript da interface de edição de receitas}
	\label{client_rearrange}
\end{figure}

Este teste foi repetido para obter um número de 10 amostras dos tempos de execução total do frame, de cada iteração da função \textit{rearrange} e do tempo necessário para salvar a receita, por meio da função \textit{saveOnDemand}. Os resultados são apresentados na tabela \ref{rearrange_on_demand}.

\begin{table}[H]
	\centering
	\captionsetup{justification=centering}
	\caption[Tempos relativos a processamento do editor de receitas, no cliente]{Tempos relativos a processamento do editor de receitas, no cliente}
	\label{rearrange_on_demand}
	\begin{tabular}{ | M{4.5cm} | M{1.75cm} | M{1.75cm} | M{1.75cm} | M{1.75cm} | M{1.75cm} |}
		\hline
		\textbf{Descrição} & \textbf{Média (ms)} & \textbf{Máximo (ms)} & \textbf{Mínimo (ms)} & \textbf{Desvio padrão (ms)} & \textbf{Coef. de variação (\%)}\\ \hline
		Tempo do frame & 64,00 & 78,30 & 47,10 & 9,46 & 14,78\\ \hline
		Tempo total \textit{rearrange} 1$^a$ iteração & 18,78 & 25,09 & 14,15 & 3,78 & 20,13\\ \hline
		Tempo de processamento \textit{rearrange} 1$^a$ iteração & 0,64 & 1,52 & 0,16 & 0,41 & 64,06\\ \hline
		Tempo total \textit{rearrange} 2$^a$ iteração & 14,95 & 19,91 & 10,54 & 3,30 & 22,07\\ \hline
		Tempo de processamento \textit{rearrange} 2$^a$ iteração & 0,57 & 1,1 & 0,17 & 0,30 & 52,63\\ \hline
		Tempo total \textit{rearrange} 3$^a$ iteração & 11,85 & 15,22 & 8,41 & 2,10 & 17,72\\ \hline
		Tempo de processamento \textit{rearrange} 3$^a$ iteração & 0,56 & 0,76 & 0,18 & 0,20 & 35,71\\ \hline
		Tempo total \textit{rearrange} 4$^a$ iteração & 9,35 & 12,05 & 7,17 & 1,61 & 17,22\\ \hline
		Tempo de processamento \textit{rearrange} 4$^a$ iteração & 0,58 & 1,00 & 0,16 & 0,24 & 41,38\\ \hline
		Tempo total \textit{rearrange} 5$^a$ iteração & 7,44 & 9,48 & 5,59 & 1,23 & 16,53\\ \hline
		Tempo de processamento \textit{rearrange} 5$^a$ iteração & 0,65 & 0,98 & 0,33 & 0,24 & 36,92\\ \hline
		Tempo total \textit{rearrange} 6$^a$ iteração & 5,39 & 6,85 & 4,03 & 0,88 & 16,33\\ \hline
		Tempo de processamento \textit{rearrange} 6$^a$ iteração & 0,59 & 0,98 & 0,29 & 0,21 & 35,59\\ \hline
		Tempo total \textit{rearrange} 7$^a$ iteração & 3,12 & 3,96 & 2,20 & 0,61 & 19,55\\ \hline
		Tempo de processamento \textit{rearrange} 7$^a$ iteração & 0,76 & 1,95 & 0,32 & 0,45 & 59,21\\ \hline
		Tempo total \textit{saveOnDemand} & 5,01 & 7,16 & 3,85 & 1,11 & 22,16\\ \hline
		Tempo de processamento \textit{saveOnDemand} & 0,23 & 0,54 & 0,14 & 0,13 & 56,52\\ \hline
	\end{tabular}
\end{table}

A partir dos resultados apresentados, foi notado que tanto a função \textit{rearrange} quanto \textit{saveOnDemand} estavam sendo executadas duas vezes, impondo uma redundância não prevista à interface de edição de receitas e, por conseguinte, consumindo recursos em dobro desta --- além de isto significar o dobro de operações de escrita na memória eMMC da BBB.

Quanto ao tempo médio total de execução da função \textit{rearrange}, foi notado que este é diminuído para cada iteração: tal resultado era esperado, já que o tempo total da n-ésima iteração é o somatório do tempo de execução desta e de todas as iterações subsequentes. Já o tempo de processamento para cada iteração não diminuiu, o que também é esperado, uma vez que a função \textit{rearrange} não tem laços de repetição internos que dependam do valor n da n-ésima iteração. Ainda assim, para todos os valores médios da tabela \ref{rearrange_on_demand}, percebeu-se que estes apresentam baixa confiaça estatística, dados os valores dos coeficientes de variação não desprezíveis.
