...
\begin{enumerate}
	...
	\item Esperar um dos leds de status acender antes de liberar o botão
...
Na edição do arquivo \textit{/etc/ntp.conf}, a única modificação feita foi a substituição dos servidores de hora padrão para os servidores brasileiros, listados no endereço \url{http://ntp.br}, responsável pelo oferecimento da \textbf{Hora Legal Brasileira} e cuja estrutura de oferecimento do serviço está apresentada na figura \ref{ntp_stratos}. No final de cada endereço foi adicionada a instrução \textit{iburst}, conforme descrito no website oficial do protocolo (\url{http://support.ntp.org/bin/view/Support/ConfiguringNTP}) e que, embora não esteja claro seu funcionamento, foi essencial para que o horário fosse corretamente configurado a cada \textit{reboot}. O trecho de código-fonte \ref{ntp_br} indica as modificações no arquivo \textit{/etc/ntp.conf} adequando aos servidores brasileiros:

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.75]{./Resources/ntp-estrutura.png}
	\captionsetup{justification=centering}
	\caption[Estrutura do serviço de data e hora do Ntp.br]{Estrutura do serviço de data e hora do Ntp.br\\ Disponível em \url{http://ntp.br/img-ntp/ntp-estrutura.png}}
	\label{ntp_stratos}
\end{figure}
